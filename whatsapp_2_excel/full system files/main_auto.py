...
Version: 3.0 - Full Automation: Folder Watcher + Auto-Propagation + Persistent Settings
"""
...
# ============================================================================
# PERSISTENT SETTINGS
# Saves Excel path and watch folder so they survive restarts.
# Stored in main_settings.json next to this script — no hardcoded paths.
# ============================================================================

_SETTINGS_FILE = Path(__file__).parent / "main_settings.json"
...
class Config:
    ...
    def __post_init__(self):
        # Load saved Excel path on startup — no hardcoding needed
        if not self.excel_file:
            self.excel_file = _SETTINGS.get("excel_file", "")

    def save_excel_path(self):
        """Persist the chosen Excel path."""
        _SETTINGS["excel_file"] = self.excel_file
...
class AutomationController:
    ...
    def run(self,
        ...
        """Run the complete automation process: parse → update → propagate."""
        ...
        # Auto-propagation — runs immediately after every successful update
        if update_result.added > 0:
            logging.info("🔄 Auto-propagating previous dates, diesel levels & DG hours...")
            try:
                prop_result = self.propagator.propagate_data(progress_callback=progress_callback)
                logging.info(
                    f"✅ Auto-propagation done: {prop_result.sites_processed} sites | "
                    f"{prop_result.rows_updated} rows updated | "
                    f"{prop_result.dates_filled} dates | "
                    f"{prop_result.diesel_levels_filled} diesel levels | "
                    f"{prop_result.dg_hours_filled} DG hours"
                )
            except Exception as e:
                logging.warning(f"⚠️ Auto-propagation encountered an issue: {e}")
...
class AutomationGUI:
    ...
    def _setup_ui(self):
        ...
        # Only set title/geometry if root is an actual Tk window, not a frame
        if isinstance(self.root, tk.Tk):
            self.root.title("WhatsApp to Excel Automation v3.0")
        ...
        # Excel file label — shows saved path on startup
        saved_excel = self.config.excel_file
        saved_name  = os.path.basename(saved_excel) if saved_excel else "Not selected — click 📊 to browse"
        self.excel_label = ttk.Label(self.root, text=f"Excel: {saved_name}",
                                     font=("Arial", 9), foreground="gray")
        self.excel_label.grid(row=5, column=0, pady=2)
        if saved_excel:
            self.status_var.set("Excel loaded from last session — select chat file to begin")

        # Info label
        info_text = "v3.0 • Folder Watcher + Auto-Propagation + Persistent Settings"
        ttk.Label(self.root, text=info_text, font=("Arial", 8), foreground="blue").grid(row=6, column=0, pady=5)
    ...
    def select_excel_file(self):
        ...
        if path:
            ...
            self.config.save_excel_path()   # persist for next restart
            filename = os.path.basename(path)
            self.excel_label.config(text=f"Excel: {filename}")
            logging.info(f"📊 Excel file selected & saved: {filename}")
...
# Drop a .txt WhatsApp export into the watched folder and the Excel updates
# automatically — parse + update + propagate all happen without any clicks.
# ============================================================================

_watcher_excel_lock = threading.Lock()


if WATCHDOG_AVAILABLE:
    class ChatFileHandler(FileSystemEventHandler):
        def __init__(self, config: "Config", log_cb: Callable, err_cb: Callable):
            super().__init__()
            self.config   = config
            self.log_cb   = log_cb
            self.err_cb   = err_cb
        ...
        def on_created(self, event):
            ...
            threading.Timer(1.5, self._handle, args=[path]).start()

        def _handle(self, path: Path):
            if not path.exists():
                return
            if not self.config.excel_file:
                self.err_cb("⚠️ No Excel file set — select one in the main tab first.")
                return

            self.log_cb(f"📂 Detected: {path.name}")
            try:
                controller = AutomationController(self.config)
                with _watcher_excel_lock:
                    result, skipped = controller.run(
                        str(path),
                        error_logger=self.err_cb
                    )
                self.log_cb(
            ...
            try:
                ...
                self.log_cb(f"📁 Moved to: processed/{dest.name}")
            except Exception as e:
                self.err_cb(f"Could not move file: {e}")


class FolderWatcherTab:
    """Tab added to AutomationGUI for fully automatic file watching."""

    def __init__(self, notebook: ttk.Notebook, root: tk.Tk, config: "Config"):
        self.root     = root
        self.config   = config
        self.observer = None
        self.watching = False
        ...
        notebook.add(self.frame, text="📂 Auto Watcher")
        self._build()

    def _build(self):
        # Watch folder row
        ff = ttk.LabelFrame(self.frame, text="Watch Folder", padding=8)
        ff.pack(fill="x", pady=(0, 8))
        saved_folder = _SETTINGS.get("watch_folder", "")
        self.folder_var = tk.StringVar(value=saved_folder or "No folder selected")
        ttk.Label(ff, textvariable=self.folder_var, width=72, anchor="w").pack(side="left", padx=5)
        ttk.Button(ff, text="Browse…", command=self._browse).pack(side="right", padx=5)

        # How-to
        info = ttk.LabelFrame(self.frame, text="How it works", padding=8)
        info.pack(fill="x", pady=(0, 8))
        ttk.Label(info, justify="left", font=("Consolas", 9), text=(
            "1.  Select (or create) a watch folder\n"
            "2.  Click Start Watcher\n"
            "3.  Drop any WhatsApp .txt export into that folder\n"
            "4.  The tool will automatically:\n"
            "      - Parse the messages\n"
            "      - Add new entries to Excel\n"
            "      - Run propagation (dates, diesel, DG hours)\n"
            "      - Move the file to a processed subfolder\n\n"
            "Make sure the Excel file is selected in the main tab first."
        )).pack(anchor="w")

        # Buttons
        bf = ttk.Frame(self.frame)
        bf.pack(fill="x", pady=(0, 8))
        self.start_btn = ttk.Button(bf, text="▶  Start Watcher", command=self._start)
        self.start_btn.pack(side="left", padx=5)
        self.stop_btn  = ttk.Button(bf, text="⏹  Stop Watcher",  command=self._stop, state="disabled")
        self.stop_btn.pack(side="left", padx=5)
        ttk.Button(bf, text="Clear Log", command=self._clear).pack(side="left", padx=5)
        self.status_lbl = ttk.Label(bf, text="● Stopped", foreground="red",
                                    font=("Consolas", 10, "bold"))
        self.status_lbl.pack(side="right", padx=10)

        # Log
        lf = ttk.LabelFrame(self.frame, text="Watcher Log", padding=5)
        lf.pack(fill="both", expand=True)
        self.log_area = scrolledtext.ScrolledText(
            lf, state="disabled", height=18,
            font=("Consolas", 10), bg="#1e1e1e", fg="#00ff00"
        )
        self.log_area.pack(fill="both", expand=True)

        if not WATCHDOG_AVAILABLE:
            self._log("⚠️  watchdog not installed. Run:  pip install watchdog")
            self.start_btn.config(state="disabled")
        elif saved_folder and Path(saved_folder).exists():
            self._log(f"📁 Watch folder restored: {saved_folder}")

    def _browse(self):
        folder = filedialog.askdirectory(title="Select Watch Folder")
        if folder:
            self.folder_var.set(folder)
            _SETTINGS["watch_folder"] = folder
            _save_settings(_SETTINGS)
    ...
    def _start(self):
        ...
        if self.watching:
            return
        handler = ChatFileHandler(self.config, self._log, self._log)
        ...
        self.status_lbl.config(text="● Watching", foreground="green")
        self._log(f"👀 Watching: {folder}")
        self._log("    Drop a WhatsApp .txt export here to trigger full automation.")
    ...
    def _log(self, msg: str):
        def _append():
            ...
            self.log_area.insert(tk.END, datetime.now().strftime("%H:%M:%S") + "  " + msg + "\n")
...
def main():
    ...
    # Wrap AutomationGUI inside a notebook so both tabs co-exist
    root.title("WhatsApp to Excel Automation v3.0")
    root.geometry("1100x900")

    notebook = ttk.Notebook(root)
    notebook.pack(fill="both", expand=True)

    # Tab 1 — original manual interface
    manual_frame = ttk.Frame(notebook)
    notebook.add(manual_frame, text="⚙️  Manual")

    # Re-parent the GUI into the manual_frame
    app = AutomationGUI(manual_frame, config)

    # Tab 2 — folder watcher
    FolderWatcherTab(notebook, root, config)
